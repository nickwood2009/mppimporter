...
namespace ADC.MppImport.Services
{
    public class MppAsyncImportService
    {
        ...
        private void UpdateDurationVarianceFields(Guid projectId, ImportJobPayload payload)
        {
            ...
            foreach (var taskEntity in tasks.Entities)
            {
                ...
                if (dto.DayCount.HasValue)
                    taskUpdate["adc_daycount1"] = (double)dto.DayCount.Value;
                TrySetDecimalValue(taskUpdate, "adc_baseline1", dto.Baseline1);
                TrySetDecimalValue(taskUpdate, "adc_baseline2", dto.Baseline2);
                TrySetDecimalValue(taskUpdate, "adc_baseline3", dto.Baseline3);
                TrySetOptionSetValue(taskUpdate, "adc_assigneerole", dto.AssigneeRole, assigneeRoleMap);
                TrySetOptionSetValue(taskUpdate, "adc_milestone", dto.Milestone, milestoneMap);

                // Compute variance
                double? pssDuration = taskEntity.GetAttributeValue<double?>("msdyn_duration");
                if (pssDuration.HasValue && !dto.IsSummary)
                {
                    double varianceDays = pssDuration.Value - dto.SourceDurationDays;
        ...
        private string GetVarianceReason(TaskDto dto, double pssDuration, Entity taskEntity,
